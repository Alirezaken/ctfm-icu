...
def run(cfg, force: bool = False, intervention: str = None):
    ...
    for gamma in scfg["gamma_grid"]:
        for delta in scfg["delta_grid"]:
            for rep in range(reps):
                r = _one_cell(S, Vimg, U_perp, float(gamma), float(delta), target_rd,
                              np.random.default_rng(cfg.seed + 7919 * rep), cfg, sct, sco)
                if r is None:
                    continue
                rows.append({"sweep": "gamma_delta", "gamma": gamma, "delta": delta,
                             "redundancy": "", "replicate": rep,
                             "confounder_r2_on_structured": round(r2, 3), **r})
            done = [x for x in rows if x["gamma"] == gamma and x["delta"] == delta]
            if done:
    ...
    for rho in scfg["redundancy_grid"]:
        for rep in range(reps):
            ...
            r = _one_cell(S, Vimg, U_mix, g, d, target_rd, rr, cfg, sct, sco)
            if r is None:
                continue
            rows.append({"sweep": "redundancy", "gamma": g, "delta": d,
                         "redundancy": rho, "replicate": rep,
                         "confounder_r2_on_structured": round(r2, 3), **r})
        done = [x for x in rows if x["sweep"] == "redundancy" and x["redundancy"] == rho]
        if done:
    ...
    R.append_rows(cfg, "synthetic.csv", rows)
    ...
    cell = [x for x in rows if x["sweep"] == "gamma_delta"
            and x["gamma"] == ga and x["delta"] == de]
    if cell:
        got = float(np.mean([x["bias_reduction"] for x in cell]))
    ...
    log(f"synthesize done in {time.time()-t0:,.0f}s -> synthetic.csv ({len(rows)} rows)")
